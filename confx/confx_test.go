...
func TestProvideForbidsSecretInFile(t *testing.T) {
	path := filepath.Join(t.TempDir(), "config.toml")
	body := "[widgets.alpha]\nendpoint = \"e\"\nlabel = \"l\"\ntoken = \"in-file\"\n"
	if err := os.WriteFile(path, []byte(body), 0o600); err != nil {
		t.Fatalf("write toml: %v", err)
	...
	// token maps to a koanf:"-" field, so it is an unknown key in the file.
	_, err := runProvide(t, path, "widgets.alpha", "alpha")
	if err == nil {
		t.Fatal("expected an error when a secret is placed in the file")
	}
}
